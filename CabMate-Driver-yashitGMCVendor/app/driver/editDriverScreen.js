...
const EditDriversScreen = () => {
  ...
  const [phoneNumber, setPhoneNumber] = useState(item?.mno);
  const [altNumber, setaltNumber] = useState(item?.altno);
  const [name, setName] = useState(item?.name);
  const [training, settraining] = useState(item?.training);
  return (
  ...
  function profilePhotoInfo() {
    return (
      <View style={{ margin: Sizes.fixPadding * 2.0 }}>
        <Text style={{ ...Fonts.grayColor15SemiBold }}>Profile Image</Text>
        <TouchableOpacity
          activeOpacity={0.1}
        ...
        >
          ...
          />
          {imagePicker(setProfileImage, profileImg, "Upload Profile Pic")}
        </TouchableOpacity>
        {divider()}
      </View>
    );
